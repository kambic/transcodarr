...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        parser.add_argument(
            "name", nargs="?", help="Name of a file or folder in the tree."
        )
        parser.add_argument("--preset", help="Preset slug.")
        parser.add_argument(
            "--presets", action="store_true", help="List presets and exit."
        )
        parser.add_argument(
            "--all-media", action="store_true", help="Every media file in the tree."
        )
        parser.add_argument(
            "--dry-run", action="store_true", help="Show what would be queued."
        )
        parser.add_argument(
            "--wait", action="store_true", help="Run jobs inline instead of enqueueing."
        )
    ...
    def handle(self, *args, **options):
        if options["presets"]:
            for preset in Preset.objects.all():
                ...
                self.stdout.write(
                    f"{mark} {preset.slug:<16} {preset.label} ({preset.summary})"
                )
            return

        if not options["preset"]:
            raise CommandError(
                "Pass --preset SLUG, or --presets to see what's available."
            )
        try:
        ...
        if options["dry_run"]:
            for node in targets:
                self.stdout.write(
                    f"would convert {node.name} → {preset.output_name(node.name)}"
                )
            self.stdout.write(
                self.style.WARNING(f"{len(targets)} file(s), nothing queued.")
            )
            return
        ...
        for job in report["queued"]:
            if options["wait"]:
                ...
                style = (
                    self.style.SUCCESS if done.status == "done" else self.style.ERROR
                )
                self.stdout.write(
                    style(f"{done.source.name}: {done.status} — {done.headline()}")
                )
            else:
