...
class PresetAdmin(admin.ModelAdmin):
    list_display = ("label", "slug", "media_kind", "summary", "container", "enabled", "position")
    list_filter = ("media_kind", "enabled")
    prepopulated_fields = {"slug": ("label",)}
    fieldsets = (
        (None, {"fields": ("label", "slug", "description", "media_kind", "enabled", "position")}),
...
class TranscodeJobAdmin(admin.ModelAdmin):
    list_display = ("created_at", "source", "preset", "status", "progress",
                    "savings", "elapsed_seconds", "requested_by")
    list_filter = ("status", "preset")
    date_hierarchy = "created_at"
    raw_id_fields = ("source", "output", "destination", "operation")
    readonly_fields = ("command", "log", "error", "task_id", "progress",
...
class MediaProbeAdmin(admin.ModelAdmin):
    list_display = ("node", "resolution", "runtime", "video_codec", "audio_codec", "probed_at")
