...
(() => {
  ...
  var continue_watching_splide = null;
  var bookmarks_splide = null;
  ...
  function stopWatching(voyokey) {
    app.bookmarks.voyoBookmarkRemove("stayedAt", voyokey).then(() => {
      if (continue_watching_splide) {
        var slideIndex = continue_watching_splide.Components.Slides.get().findIndex(slide => {
  ...
  function removeBookmark(voyokey) {
    app.bookmarks.voyoBookmarkRemove("GRP_DEFAULT", voyokey).then(() => {
      if (bookmarks_splide) {
        var slideIndex = bookmarks_splide.Components.Slides.get().findIndex(slide => {
          return slide.slide.dataset.uniq === voyokey;
        });
        if (slideIndex !== -1) {
      ...
      var slides = app.html.qAll('.splide__slide[data-uniq="' + voyokey + '"]');
