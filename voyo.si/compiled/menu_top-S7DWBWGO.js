...
(() => {
  ...
  let debouncedSendEvent = null;
  let profileMenuHideTimeout = 0;
  ...
  function hideSettingsPopup(timeout = 1e3) {
    if (!window.app) {
      return;
    }
    window.clearTimeout(profileMenuHideTimeout);
  ...
  function loginToProfile(event) {
    ...
    const el = event.target.closest("[data-profile_id]");
    if (!el) {
      return;
    }
    const profileId = app.html.getData(el, "profile_id");
  ...
  function showCurrentProfile(profiles) {
    ...
    const avatarImg = app.html.q("#current_profile_avatar");
    if (avatarImg) {
  ...
  document.addEventListener("click", (event) => {
    var _a;
    const wrapper = (_a = event.target.closest(".menu-mobile--hamburger")) == null ? void 0 : _a.closest(".menu-mobile--wrapper");
    if (!wrapper) return;
    event.preventDefault();
    wrapper.classList.toggle("menu-mobile--open");
  });
  function redirectIfNotOnSearchPage(query = "") {
