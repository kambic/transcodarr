...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        parser.add_argument("--only", choices=STEPS, action="append", help="Run just these steps.")
    ...
    def handle(self, *args, **options):
        ...
        plan = {
            "trash": ("purge trash older than %d days" % options["trash_days"],
                      lambda: purge_expired_trash.enqueue(options["trash_days"])),
            "log": ("prune the operation log to %d rows" % options["keep_operations"],
                    lambda: prune_operation_log.enqueue(options["keep_operations"])),
            "stuck": ("fail transcode jobs stalled over %d minutes" % options["stuck_minutes"],
                      lambda: sweep_stuck_jobs.enqueue(options["stuck_minutes"])),
            "probe": ("probe up to %d unmeasured media files" % options["probe_limit"],
        ...
        for name in steps:
            ...
            except Exception as exc:  # noqa: BLE001 — one bad step shouldn't stop the others
                self.stderr.write(self.style.ERROR(f"{name}: {type(exc).__name__}: {exc}"))
